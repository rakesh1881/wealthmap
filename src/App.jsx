// ============================================================
// WEALTHMAP v72 — Brokerage deducted from source account on buy (added to sell credit reduction)
// Changes vs v16:
...
const STORAGE_KEY  = "wealthmap_v72";
  ...
  sorted.forEach(tx => {
    ...
    if (!map[key]) {
      ...
      map[key] = {
        ...
        type:           (tx.type === "demerger_buy" ? (existing?.type || "stock") : tx.invType) || existing?.type || "stock",
    ...
    if (tx.type === "buy" || tx.type === "demerger_buy") {
      ...
      const price = tx.type === "demerger_buy"
        ...
        (ca.result_stocks || []).forEach(rs => {
          ...
          const emitBatch = (price, date) => {
            ...
            virtualTxs.push({
              ...
              holdingId:    `h_dem_${ca.id}_${outSym}_${outAccId}_b${batchNum}`,
...
function TradeHistoryModal({ symbol, holding, investmentTx, accounts, state, dispatch, onClose }) {
  ...
  const allTrades = (investmentTx||[])
    .filter(t => {
      const matchesHolding = holding
        ...
        : t.symbol===symbol;
      // Include real trades + demerger_buy (new stock lots) + demerger_cost_reduce (parent cost adj)
      // Exclude only demerger_sell (virtual zero-out of parent on full demerger)
      // Include: real buys/sells, demerger_buy (output lots), demerger_cost_reduce (parent cost adj)
      // Exclude: demerger_sell (virtual zero-out on full demerger)
      return matchesHolding && t.type !== "demerger_sell";
    })
    .sort((a,b)=>new Date(a.date)-new Date(b.date));
  ...
  for (const t of allTrades) {
    ...
    if (t.type === "buy" || t.type === "demerger_buy") {
      lots.push({
        id:        t.id,
        date:      t.date,
        type:      t.type,
...
function CaMasterModal({ state, dispatch, onClose, checkForPendingCAs }) {
  ...
  const CA_TYPE_LABELS = {
    ...
    stock_name_change:"✏️ Name Change",
  };
                            ...
                            <div style={{flex:1}}>
                              ...
                              {virtualTxs.length>0&&(
                                <div style={{fontSize:11,color:"#6366F1",marginTop:4}}>
                                  {virtualTxs.filter(t=>t.type==="demerger_buy").length} demerger lot(s) generated
                                </div>
                              )}
                            </div>
                            <button onClick={()=>{
