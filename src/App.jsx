// ============================================================
// WEALTHMAP v52 — Nav: Notes → Accounts in bottom nav
// Changes vs v16:
...
const STORAGE_KEY  = "wealthmap_v52";
    ...
    case "ADD_CORPORATE_ACTION": {
      ...
      if (ca.action_type === "stock_split" || ca.action_type === "bonus") {
        ...
        (ca.result_stocks || []).forEach(rs => {
          ...
          let lotQueue = eligLots.map(l => ({ ...l })); // clone
          let batchNum  = 0;
          let batchQty  = 0;   // parent shares accumulated in current batch
          let batchCost = 0;   // allocated cost (at costPct%) for current batch

          const flushBatch = (batchQ, batchC, batchDate) => {
            const newSharesFromBatch = Math.floor((batchQ / ratioFrom) * ratioTo);
            if (newSharesFromBatch <= 0) return;
            const pricePerNewShare = newSharesFromBatch > 0 ? batchC / newSharesFromBatch : 0;
            batchNum++;
            virtualTxs.push({
              ...
              symbol:       outSym,
              name:         outName,
              quantity:     newSharesFromBatch,
              price:        pricePerNewShare,
              currency, date: batchDate || ca.date, accountId: outAccId,
              holdingId:    `h_dem_${ca.id}_${outSym}_${outAccId}_b${batchNum}`,
              note: `Demerger: ${newSharesFromBatch} ${outSym} from ${batchQ.toFixed(4)} ${parentSym} (ratio ${ratioFrom}:${ratioTo}, ${costPct}% cost)`,
            });
          };

          // Walk lots, fill batches of ratioFrom shares at a time
          for (const lot of lotQueue) {
            let remaining = lot.remQty;
            const lotAllocCostPerShare = lot.price * (costPct / 100); // cost per parent share allocated to new stock

            while (remaining > 0.0001) {
              const need       = ratioFrom - batchQty;
              const take       = Math.min(need, remaining);
              batchQty        += take;
              batchCost       += take * lotAllocCostPerShare;
              remaining       -= take;

              if (Math.abs(batchQty - ratioFrom) < 0.0001) {
                // Full batch complete — flush it using this lot's date
                flushBatch(batchQty, batchCost, lot.date);
                batchQty  = 0;
                batchCost = 0;
              }
            }
...
function useCaAutoDetect(state, dispatch) {
  ...
  function checkForPendingCAs(symbols) {
    const allSymbols = symbols && symbols.length > 0
      ? symbols
      : [...new Set((state?.investmentTx||[]).map(t => t.symbol).filter(Boolean))];

    if (allSymbols.length === 0) return;

    const alreadyApplied = new Set(
      (state?.corporateActions||[]).map(ca => ca._masterRef).filter(Boolean)
    );
    const pending = (state?.corporateActionsMaster||[]).filter(m => {
      if (!allSymbols.includes(m.symbol)) return false;
      if (alreadyApplied.has(m.id)) return false;
      if (dismissedIds.has(m.id)) return false;
      const eligibleTrades = (state?.investmentTx||[]).filter(
  ...
  // ── Auto-scan: whenever CA master changes (new entry added) OR on mount ────
  // This catches pre-existing holdings that match a newly added CA master entry.
  const caMasterLen = (state?.corporateActionsMaster||[]).length;
  useEffect(() => {
    checkForPendingCAs(null); // null = scan ALL symbols in investmentTx
  }, [caMasterLen]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Also scan on first load if master has entries ─────────────────────────
  const [didInitialScan, setDidInitialScan] = useState(false);
  useEffect(() => {
    if (!didInitialScan && caMasterLen > 0 && (state?.investmentTx||[]).length > 0) {
      checkForPendingCAs(null);
      setDidInitialScan(true);
    }
  }, [caMasterLen, (state?.investmentTx||[]).length, didInitialScan]); // eslint-disable-line react-hooks/exhaustive-deps
...
function TradeHistoryModal({ symbol, holding, investmentTx, accounts, state, dispatch, onClose }) {
  ...
  const allTrades = (investmentTx||[])
    .filter(t => {
      const matchesHolding = holding
        ...
        : t.symbol===symbol;
      return matchesHolding && t.invType !== "demerger_virtual";
    })
    .sort((a,b)=>new Date(a.date)-new Date(b.date));

  // Build FIFO lots from buy trades, then consume sells
  // Each lot: { tradeId, date, price, currency, origQty, remainQty, note }
  const lots = [];
  for (const t of allTrades) {
    ...
    // demerger_sell / demerger_cost_reduce — skip, handled by virtual tx system
  }

  // Only lots with remaining quantity
  const remainingLots = lots.filter(l => l.remainQty > 0.0001);
