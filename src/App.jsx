// ============================================================
// WEALTHMAP v68 — Brokerage deducted from source account on buy (added to sell credit reduction)
// Changes vs v16:
...
const STORAGE_KEY  = "wealthmap_v68";
    ...
    case "ADD_CORPORATE_ACTION": {
      ...
      if (ca.action_type === "stock_split" || ca.action_type === "bonus") {
        ...
        (ca.result_stocks || []).forEach(rs => {
          ...
          // We walk through lots sequentially, accumulating qty and cost

          // ── WAVG-reset algorithm (matches Zerodha's demerger cost basis) ──────────
          // Accumulate a running pool of shares + cost.
          // When pool reaches ratioFrom shares → emit 1 ITCHOTELS batch at pool's WAVG price.
          // After emit: remove 10 shares worth at WAVG from pool (keep remainder).
          // This means each batch uses the weighted average cost of ALL shares
          // accumulated up to that point (including leftovers from prior batches).
          let batchNum    = 0;
          let poolQty     = 0;   // total parent shares currently in pool
          let poolCost    = 0;   // total cost of shares in pool (at 100%, not costPct%)
          let lastDate    = ca.date;

          const flushBatch = (bDate) => {
            const wavg             = poolQty > 0 ? poolCost / poolQty : 0;
            const newSharesPerBatch = Math.floor(ratioTo);  // ratioTo per ratioFrom shares
            const pricePerNewShare  = (wavg * costPct / 100) * ratioFrom / Math.max(1, ratioTo);
            batchNum++;
            virtualTxs.push({
              ...
              price:        pricePerNewShare,
              currency, date: bDate || ca.date, accountId: outAccId,
            ...
            // Remove ratioFrom shares worth of cost at WAVG — keep remainder in pool
          ...
          if (batchQty > 0.0001) {
            // These shares don't complete a full batch — their allocated cost is lost/residual
            // Record as a zero-qty informational tx (won't affect holdings)
            const residualCost = batchCost;
            virtualTxs.push({
              ...
              currency, date: ca.date, accountId: accId,
              holdingId:    parentHolding.id || ("h_" + parentSym + "_" + accId),
              note: `Demerger residual: ${batchQty.toFixed(4)} ${parentSym} shares (< ${ratioFrom}:${ratioTo} ratio) — ₹${residualCost.toFixed(2)} allocated cost retained in ${parentSym}`,
            });
          }
        });
