// ============================================================
// WEALTHMAP v116 — Sub-categories stored in wm_sub_categories table (not JSONB)
// Changes vs v16:
...
const STORAGE_KEY  = "wealthmap_v116";
...
function rowToExpenseCategory(r) { return { id: r.id, name: r.name||"", icon: r.icon||"", subCategories: [] }; }
function rowToIncomeCategory(r)  { return { id: r.id, name: r.name||"", icon: r.icon||"", subCategories: [] }; }
function rowToSubCategory(r)     { return { id: r.id, parentId: r.parent_id, kind: r.kind, name: r.name||"", icon: r.icon||"" }; }
...
async function pushCloud(userId, s) {
  ...
  // Collect all parent cats (expense + income) whose subCategories changed
  ...
  if (subCatParentsToClean.length > 0) {
    // Delete all old sub-cats for affected parents
    ops.push((async()=>{
    ...
    // Re-insert current sub-cats for changed (not deleted) parents
    ...
    if (isMerger) {
      ...
      for (const lot of eligLots) {
        ...
        while (poolQty >= ratioFrom-0.0001) {
          const slice=poolCost/poolQty*ratioFrom; bNum++;
          allVirtual.push({ id:"mgr_buy_"+ca.id+"_"+targetSym+"_b"+bNum, type:"merger_buy",
            invType:"demerger_virtual", demergerCaId:ca.id, symbol:targetSym, name:targetSym,
            quantity:ratioTo, price:Math.round(slice/ratioTo*100)/100,
            currency, date:recordDate, accountId:accId, holdingId:tgtHoldingId, note:"Merger batch "+bNum });
          poolCost-=slice; poolQty-=ratioFrom;
        }
      }
      if (poolQty>0.0001) { const res=Math.floor(poolQty/ratioFrom*ratioTo); if(res>0){ bNum++;
        allVirtual.push({ id:"mgr_buy_"+ca.id+"_"+targetSym+"_b"+bNum, type:"merger_buy",
          invType:"demerger_virtual", demergerCaId:ca.id, symbol:targetSym, name:targetSym,
          quantity:res, price:Math.round(poolCost/poolQty*(ratioFrom/ratioTo)*100)/100,
          currency, date:recordDate, accountId:accId, holdingId:tgtHoldingId, note:"Merger residual" });
      }}
      ...
      if (ca.action_type === "stock_split" || ca.action_type === "bonus") {
        ...
        if (ca.action_type === "merger") {
          ...
          if (targetSym) {
            ...
            // Batch FIFO: pool lots → flush every mgrRatioFrom source shares into mgrRatioTo target shares
            let poolQty  = 0;
            let poolCost = 0;
            let batchNum = 0;
            ...
            for (const lot of eligibleLots) {
              poolQty  += lot.qty;
              poolCost += lot.qty * lot.price;

              while (poolQty >= mgrRatioFrom - 0.0001) {
                const usedSrc       = mgrRatioFrom;
                const costSlice     = (poolCost / poolQty) * usedSrc;
                const pricePerTgt   = costSlice / mgrRatioTo;
                batchNum++;
                virtualTxs.push({
                  id:           "mgr_buy_" + ca.id + "_" + targetSym + "_b" + batchNum,
                  type:         "merger_buy",
                  invType:      "demerger_virtual",
                  demergerCaId: ca.id,
                  ...
                  currency, date: recordDate, accountId: accId, holdingId: tgtHoldingId,
                  note: "Merger batch " + batchNum + ": " + mgrRatioTo + " " + targetSym + " from " + mgrRatioFrom + " " + parentSym,
                });
                poolCost -= costSlice;
                poolQty  -= usedSrc;
              }
            }
            // Residual fractional batch
            if (poolQty > 0.0001) {
              const residualTgt = Math.floor(poolQty / mgrRatioFrom * mgrRatioTo);
              if (residualTgt > 0) {
                const pricePerTgt = (poolCost / poolQty) * (mgrRatioFrom / mgrRatioTo);
                batchNum++;
                virtualTxs.push({
                  id:           "mgr_buy_" + ca.id + "_" + targetSym + "_b" + batchNum,
                  type:         "merger_buy", invType: "demerger_virtual", demergerCaId: ca.id,
                  symbol:       targetSym, name: targetSym,
                  quantity:     residualTgt,
                  price:        Math.round(pricePerTgt * 100) / 100,
                  currency, date: recordDate, accountId: accId, holdingId: tgtHoldingId,
                  note: "Merger residual: " + residualTgt + " " + targetSym,
                });
              }
            }
    ...
    (async()=>{
      ...
      try {
        ...
        if (cloud) {
          ...
          // ── Reapply all CAs to regenerate virtual txs (merger_sell, demerger_buy etc.)
          // Virtual txs are not persisted to DB — they are derived on load from the CA list.
          const realTxsOnLoad = (cloud.investmentTx || []).filter(t => t.invType !== "demerger_virtual");
          const virtualTxsOnLoad = reapplyAllCAs(cloud.corporateActions || [], realTxsOnLoad, cloud.holdings || [], cloud.corporateActionsMaster || []);
          if (virtualTxsOnLoad.length > 0) {
            console.log("[DB] Re-derived", virtualTxsOnLoad.length, "virtual CA txs from", (cloud.corporateActions||[]).length, "applied CAs");
            const allTxsOnLoad = [...realTxsOnLoad, ...virtualTxsOnLoad];
